...
class TestSDOVariables(unittest.TestCase):
    ...
    def test_array_iter_length(self):
        """Assume the "highest subindex supported" entry is not counted."""
        run(self._test_array_iter_length())

    async def _test_array_iter_length(self):
        array = self.sdo_node[0x1003]
        subs = len([1 async for _ in array])
        self.assertEqual(await array.get_length(), 3)
        self.assertEqual(subs, 3)
        # Simulate more entries getting added dynamically
        await array[0].set_data(b'\x08')
        subs = len([1 async for _ in array])
    ...
    def test_array_members_dynamic(self):
        ...
        run(self._test_array_members_dynamic())

    async def _test_array_members_dynamic(self):
        array = self.sdo_node[0x1003]
        async for subindex in array:
            self.assertIsInstance(array[subindex], canopen.sdo.SdoVariable)

    def test_array_contains_non_int(self):
        """SdoArray.contains() should handle non-int types gracefully."""
        run(self._test_array_contains_non_int())

    async def _test_array_contains_non_int(self):
        array = self.sdo_node[0x1003]
        self.assertFalse(await array.contains("not an int"))
        self.assertFalse(await array.contains(None))
...
class TestSDO(unittest.TestCase):
    ...
    async def _send_message(self, can_id, data, remote=False):
    ...
    def test_expedited_upload(self):
        run(self._test_expedited_upload())

    async def _test_expedited_upload(self):
        self.data = [
            (TX, b'\x40\x18\x10\x01\x00\x00\x00\x00'),
            (RX, b'\x43\x18\x10\x01\x04\x00\x00\x00')
        ]
        vendor_id = await self.network[2].sdo[0x1018][1].read()
        ...
        trans_type = await self.network[2].sdo[0x1400]['Transmission type RPDO 1'].read()
    ...
    def test_size_not_specified(self):
        run(self._test_size_not_specified())

    async def _test_size_not_specified(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x1400, 2)
        self.assertEqual(data, b'\xfe\x00\x00\x00')
        self.assertTrue(self.message_sent)

    def test_expedited_download(self):
        run(self._test_expedited_download())

    async def _test_expedited_download(self):
        self.data = [
            (TX, b'\x2b\x17\x10\x00\xa0\x0f\x00\x00'),
            (RX, b'\x60\x17\x10\x00\x00\x00\x00\x00')
        ]
        await self.network[2].sdo[0x1017].write(4000)
        self.assertTrue(self.message_sent)

    def test_segmented_upload(self):
        run(self._test_segmented_upload())

    async def _test_segmented_upload(self):
        self.data = [
        ...
        device_name = await self.network[2].sdo[0x1008].read()
        self.assertEqual(device_name, "Tiny Node - Mega Domains !")

    def test_segmented_upload_too_much_data(self):
        run(self._test_segmented_upload_too_much_data())

    async def _test_segmented_upload_too_much_data(self):
    ...
    def test_segmented_download(self):
        run(self._test_segmented_download())

    async def _test_segmented_download(self):
        self.data = [
        ...
        await self.network[2].sdo['Writable string'].write('A long string')

    def test_expedited_download_zero_length(self):
        # NOTE: was named test_segmented_download_zero_length and expected a
        # segmented transfer, but SdoVariable.set_data only forces segmented
        # transfer for DOMAIN-typed variables (sdo/base.py) — 0x2000 is
        # VISIBLE_STRING, so this has always gone through the expedited path.
        # Pre-existing, unrelated to the async port (identical logic on
        # master); the old expected bytes here never reflected reality.
        run(self._test_expedited_download_zero_length())

    async def _test_expedited_download_zero_length(self):
        self.data = [
            (TX, b'\x33\x00\x20\x00\x00\x00\x00\x00'),
            (RX, b'\x60\x00\x20\x00\x00\x00\x00\x00'),
        ]
        await self.network[2].sdo[0x2000].write("")
        self.assertTrue(self.message_sent)

    def test_abort(self):
        run(self._test_abort())

    async def _test_abort(self):
        self.data = [
            (TX, b'\x40\x18\x10\x01\x00\x00\x00\x00'),
            (RX, b'\x80\x18\x10\x01\x11\x00\x09\x06')
        ]
        with self.assertRaises(canopen.SdoAbortedError) as cm:
            _ = await self.network[2].sdo[0x1018][1].read()
...
class TestSDOClientDatatypes(unittest.TestCase):
    ...
    def test_boolean(self):
        run(self._test_boolean())

    async def _test_boolean(self):
        self.data = [
            (TX, b'\x40\x01\x20\x00\x00\x00\x00\x00'),
            (RX, b'\x4f\x01\x20\x00\xfe\xfd\xfc\xfb')
        ]
        data = await self.network[2].sdo.upload(0x2000 + dt.BOOLEAN, 0)
        self.assertEqual(data, b'\xfe')

    def test_unsigned8(self):
        run(self._test_unsigned8())

    async def _test_unsigned8(self):
        self.data = [
            (TX, b'\x40\x05\x20\x00\x00\x00\x00\x00'),
            (RX, b'\x4f\x05\x20\x00\xfe\xfd\xfc\xfb')
        ]
        data = await self.network[2].sdo.upload(0x2000 + dt.UNSIGNED8, 0)
        self.assertEqual(data, b'\xfe')

    def test_unsigned16(self):
        run(self._test_unsigned16())

    async def _test_unsigned16(self):
        self.data = [
            (TX, b'\x40\x06\x20\x00\x00\x00\x00\x00'),
            (RX, b'\x4b\x06\x20\x00\xfe\xfd\xfc\xfb')
        ]
        data = await self.network[2].sdo.upload(0x2000 + dt.UNSIGNED16, 0)
        self.assertEqual(data, b'\xfe\xfd')

    def test_unsigned24(self):
        run(self._test_unsigned24())

    async def _test_unsigned24(self):
        self.data = [
            (TX, b'\x40\x16\x20\x00\x00\x00\x00\x00'),
            (RX, b'\x47\x16\x20\x00\xfe\xfd\xfc\xfb')
        ]
        data = await self.network[2].sdo.upload(0x2000 + dt.UNSIGNED24, 0)
        self.assertEqual(data, b'\xfe\xfd\xfc')

    def test_unsigned32(self):
        run(self._test_unsigned32())

    async def _test_unsigned32(self):
        self.data = [
            (TX, b'\x40\x07\x20\x00\x00\x00\x00\x00'),
            (RX, b'\x43\x07\x20\x00\xfe\xfd\xfc\xfb')
        ]
        data = await self.network[2].sdo.upload(0x2000 + dt.UNSIGNED32, 0)
        self.assertEqual(data, b'\xfe\xfd\xfc\xfb')

    def test_unsigned40(self):
        run(self._test_unsigned40())

    async def _test_unsigned40(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.UNSIGNED40, 0)
        self.assertEqual(data, b'\xb2\x01\x20\x02\x91')

    def test_unsigned48(self):
        run(self._test_unsigned48())

    async def _test_unsigned48(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.UNSIGNED48, 0)
        self.assertEqual(data, b'\xb2\x01\x20\x02\x91\x12')

    def test_unsigned56(self):
        run(self._test_unsigned56())

    async def _test_unsigned56(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.UNSIGNED56, 0)
        self.assertEqual(data, b'\xb2\x01\x20\x02\x91\x12\x03')

    def test_unsigned64(self):
        run(self._test_unsigned64())

    async def _test_unsigned64(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.UNSIGNED64, 0)
        self.assertEqual(data, b'\xb2\x01\x20\x02\x91\x12\x03\x19')

    def test_integer8(self):
        run(self._test_integer8())

    async def _test_integer8(self):
        self.data = [
            (TX, b'\x40\x02\x20\x00\x00\x00\x00\x00'),
            (RX, b'\x4f\x02\x20\x00\xfe\xfd\xfc\xfb')
        ]
        data = await self.network[2].sdo.upload(0x2000 + dt.INTEGER8, 0)
        self.assertEqual(data, b'\xfe')

    def test_integer16(self):
        run(self._test_integer16())

    async def _test_integer16(self):
        self.data = [
            (TX, b'\x40\x03\x20\x00\x00\x00\x00\x00'),
            (RX, b'\x4b\x03\x20\x00\xfe\xfd\xfc\xfb')
        ]
        data = await self.network[2].sdo.upload(0x2000 + dt.INTEGER16, 0)
        self.assertEqual(data, b'\xfe\xfd')

    def test_integer24(self):
        run(self._test_integer24())

    async def _test_integer24(self):
        self.data = [
            (TX, b'\x40\x10\x20\x00\x00\x00\x00\x00'),
            (RX, b'\x47\x10\x20\x00\xfe\xfd\xfc\xfb')
        ]
        data = await self.network[2].sdo.upload(0x2000 + dt.INTEGER24, 0)
        self.assertEqual(data, b'\xfe\xfd\xfc')

    def test_integer32(self):
        run(self._test_integer32())

    async def _test_integer32(self):
        self.data = [
            (TX, b'\x40\x04\x20\x00\x00\x00\x00\x00'),
            (RX, b'\x43\x04\x20\x00\xfe\xfd\xfc\xfb')
        ]
        data = await self.network[2].sdo.upload(0x2000 + dt.INTEGER32, 0)
        self.assertEqual(data, b'\xfe\xfd\xfc\xfb')

    def test_integer40(self):
        run(self._test_integer40())

    async def _test_integer40(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.INTEGER40, 0)
        self.assertEqual(data, b'\xb2\x01\x20\x02\x91')

    def test_integer48(self):
        run(self._test_integer48())

    async def _test_integer48(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.INTEGER48, 0)
        self.assertEqual(data, b'\xb2\x01\x20\x02\x91\x12')

    def test_integer56(self):
        run(self._test_integer56())

    async def _test_integer56(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.INTEGER56, 0)
        self.assertEqual(data, b'\xb2\x01\x20\x02\x91\x12\x03')

    def test_integer64(self):
        run(self._test_integer64())

    async def _test_integer64(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.INTEGER64, 0)
        self.assertEqual(data, b'\xb2\x01\x20\x02\x91\x12\x03\x19')

    def test_real32(self):
        run(self._test_real32())

    async def _test_real32(self):
        self.data = [
            (TX, b'\x40\x08\x20\x00\x00\x00\x00\x00'),
            (RX, b'\x43\x08\x20\x00\xfe\xfd\xfc\xfb')
        ]
        data = await self.network[2].sdo.upload(0x2000 + dt.REAL32, 0)
        self.assertEqual(data, b'\xfe\xfd\xfc\xfb')

    def test_real64(self):
        run(self._test_real64())

    async def _test_real64(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.REAL64, 0)
        self.assertEqual(data, b'\xb2\x01\x20\x02\x91\x12\x03\x19')

    def test_visible_string(self):
        run(self._test_visible_string())

    async def _test_visible_string(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.VISIBLE_STRING, 0)
        self.assertEqual(data, b'Tiny Node - Mega Domains !')

    def test_unicode_string(self):
        run(self._test_unicode_string())

    async def _test_unicode_string(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.UNICODE_STRING, 0)
        self.assertEqual(data, b'Tiny Node - Mega Domains !')

    def test_octet_string(self):
        run(self._test_octet_string())

    async def _test_octet_string(self):
        self.data = [
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.OCTET_STRING, 0)
        self.assertEqual(data, b'Tiny Node - Mega Domains !')

    def test_domain(self):
        run(self._test_domain())
    ...
    async def _test_domain(self):
        ...
        data = await self.network[2].sdo.upload(0x2000 + dt.DOMAIN, 0)
        self.assertEqual(data, b'Tiny Node - Mega Domains !')

    def test_unknown_od_32(self):
        """Test an unknown OD entry of 32 bits (4 bytes)."""
        run(self._test_unknown_od_32())

    async def _test_unknown_od_32(self):
        self.data = [
            (TX, b'\x40\xFF\x20\x00\x00\x00\x00\x00'),
            (RX, b'\x43\xFF\x20\x00\xfe\xfd\xfc\xfb')
        ]
        data = await self.network[2].sdo.upload(0x20FF, 0)
        self.assertEqual(data, b'\xfe\xfd\xfc\xfb')

    def test_unknown_od_112(self):
        """Test an unknown OD entry of 112 bits (14 bytes)."""
        run(self._test_unknown_od_112())

    async def _test_unknown_od_112(self):
        self.data = [
            (TX, b'\x40\xFF\x20\x00\x00\x00\x00\x00'),
    ...
    def test_unknown_datatype32(self):
        ...
        run(self._test_unknown_datatype32())

    async def _test_unknown_datatype32(self):
        ...
        data = await self.network[2].sdo.upload(0x2100, 0)
        self.assertEqual(data, b'\xfe\xfd\xfc\xfb')

    def test_unknown_datatype112(self):
        """Test an unknown datatype, but known OD, of 112 bits (14 bytes)."""
        run(self._test_unknown_datatype112())

    async def _test_unknown_datatype112(self):
        # Add fake entry 0x2100 to OD, using fake datatype 0xFF
        if 0x2100 not in self.node.object_dictionary:
            fake_var = ODVariable("Fake", 0x2100)
            fake_var.data_type = 0xFF
