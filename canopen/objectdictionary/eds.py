...
try:
    ...
    from canopen.micropython_configparser import NoOptionError, NoSectionError, RawConfigParser
...
def import_from_node(node_id: int, network: object):
...
def _convert_variable(node_id: int, var_type: int, value: object) -> object:
...
def _revert_variable(var_type: int, value: object) -> object:
    if value is None:
        return None
    if var_type in (datatypes.OCTET_STRING, datatypes.DOMAIN):
